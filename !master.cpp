...
void test() {
	...
	while(start <= end){
		if(dq[start] < dq[end]){
			check.push_front(dq[start]);
			check2.push_front(dq[start]);
			start++;
		}
		else if(dq[start] > dq[end]){
			check.push_back(dq[end]);
			check2.push_back(dq[end]);
			end--;
		}
		else{
			check.push_front(dq[start]);
			check2.push_back(dq[start]);
			start++;
		}
	}
